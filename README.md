...
A node is a machine (physical or virtual) running Kubernetes onto which Pods may be scheduled. The node could be the master node or one of the worker nodes . In the preceding chapter on installing Kubernetes only a single node was used. In a later chapter, Chapter 14 , we shall discuss creating a multi-node cluster with a master and worker node/s.
